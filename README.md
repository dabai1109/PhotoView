...
- 🔍 **照片细节对比工作台 (Compare Studio)**：
  - **多屏分屏对比**：支持 2 屏（左右/上下）、3 屏、4 屏网格并行对比，自动提取 EXIF 曝光参数胶囊。
  - **全局视口联动缩放与平移 (Sync Pan & Zoom)**：在一个视口放大平移，所有分屏毫秒级同步对齐焦点；支持单图解锁微调。
  - **卷帘滑动对比 (Split Slider)**：两图完全重叠，拖动分割线扫视微弱画质与色调差异。
  - **A/B 闪烁切换对比**：按住 `Tab` 快速交替闪烁，利用视觉残留极速揪出微跑焦与眨眼废片。
  - **外部照片即拖即比**：从微信、桌面或资源管理器拖入多张照片即可秒开对比，无需载入整个相册。
  - **胜出裁决 (Pick Winner)**：一键选定胜出照片并自动标记收藏。
...
| | `Esc` | 从大图/对比模式返回网格视图 / 关闭弹窗 |
| **选片与动作** | `F` | **标记收藏 / 取消收藏**（高亮金星） |
| | `X` / `Del` / `Backspace` | **删除至回收站** / 从对比中移除 |
| | `Ctrl + Z` / `Cmd + Z` | **撤销上一步操作**（支持从回收站自动还原） |
| | `1` / `2` / `3` / `4` | 快速切换筛选：全部 / 待选 / 收藏 / 已删；或激活对比槽位 |
| **细节对比工作台** | `C` | **进入 / 退出细节对比模式** |
| | `S` | **切换全局视口同步联动 (Sync Lock)** |
| | `Tab` | **A/B 闪烁快速交替对比** |
| | `W` | **胜出裁决 (Pick Winner)**：选定胜片并退出对比 |
| **画面与视图** | `Z` / `单击画面` | **100% 焦点对齐放大** / 适应屏幕缩放 |
| | `鼠标滚轮` | 大图/对比无级缩放（最高 24x） / 网格滚动 |
| | `鼠标按住拖拽` | 放大模式下自由平移视野（支持全局联动） |
